  ...
  <div>
    <div aria-live="assertive" style="z-index:500" class="pointer-events-none fixed inset-0 flex items-end px-4 py-6 sm:items-start sm:p-6">
      <div class="flex w-full flex-col items-center space-y-4">
        <Transition>
        <div v-for="toast in $layout.toasts()" :key="toast.id" @click="$layout.clearToast(toast.id)" class="transition-all cursor-pointer pointer-events-auto w-full max-w-sm overflow-hidden rounded-lg text-white bg-black shadow-lg ring-1 ring-black ring-opacity-5">
          <div class="p-4">
            <div class="flex items-start">
              ...
              <div class="ml-3 w-0 flex-1 pt-0.5">
                ...
                <p class="mt-1 text-sm text-white">{{ toast.subtitle }}</p>
              </div>
            </div>
          </div>
        </div>
    ...
    <div
      ...
      <div class="flex min-h-0 flex-1 flex-col bg-sn-secondary">
        <a href="/" class="flex h-16 flex-shrink-0 items-center px-4 bg-sn-accent" >
          <img :class="shrink ? 'w-4 h-4' : 'h-8 w-8'" v-if="$admin.logo" :src="$admin.logo" />
          <p class="ml-4 font-bold uppercase text-white">{{ $admin.name }}</p>
        </a>
          ...
          <div class="max-w-lg mr-10">
            <div class="relative">
              ...
              <input
                ...
                id="search"
                name="search"
                class="block w-full rounded-md border-0 bg-white py-1.5 pl-10 pr-3 text-gray-900 ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-sn-primary sm:text-sm sm:leading-6"
                placeholder="Rechercher"
                @click="paletteIsOpen = !paletteIsOpen"
                type="search" />
            </div>
          </div>
          <p class="mt-1">{{ user.data && user.data.email }}</p>
